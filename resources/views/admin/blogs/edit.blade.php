<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">Modify and update your blog post</p>
            </div>
        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl rounded-2xl border border-gray-100">
                <div class="p-8 lg:p-12">
                    <form action="{{ route('admin.blogs.update', $blog->id) }}" method="POST" enctype="multipart/form-data" id="blog-form" class="space-y-8">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="title" id="title" maxlength="255" value="{{ old('title', $blog->title) }}"
                                   class="block w-full px-4 py-3 text-gray-900 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200 shadow-sm"
                        ...
                        <div class="form-group">
                            ...
                            @if($blog->image)
                                <div class="mb-4">
                                    <img src="{{ filter_var($blog->image, FILTER_VALIDATE_URL) ? $blog->image : asset('storage/'.$blog->image) }}" 
                                         alt="Current image" class="w-32 h-20 object-cover rounded-lg border border-gray-200 shadow-sm">
                                    <p class="text-[10px] text-gray-500 mt-1">Current image preview</p>
                                </div>
                            @endif
                            ...
                            <div class="flex flex-col md:flex-row gap-4">
                                ...
                                <div class="flex-1">
                                    <div class="relative">
                                        <input type="url" name="image_url" id="image_url" value="{{ old('image_url', filter_var($blog->image, FILTER_VALIDATE_URL) ? $blog->image : '') }}"
                                               class="block w-full px-4 py-3 text-sm text-gray-900 bg-white border border-gray-200 rounded-xl"
                                               placeholder="Or update with URL...">
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="meta_description" id="meta_description" rows="2" maxlength="160"
                                      class="block w-full px-4 py-3 text-gray-900 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200"
                                      placeholder="Brief summary for search engines...">{{ old('meta_description', $blog->meta_description) }}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="content" id="content" rows="12" 
                                      class="block w-full px-4 py-4 text-gray-900 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200 font-mono text-sm shadow-sm"
                                      placeholder="Write your blog content here...">{{ old('content', $blog->content) }}</textarea>
                            @error('content')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                        </div>

                        <!-- Publishing Options -->
                        <div class="bg-gray-50 rounded-2xl p-6 border border-gray-100 flex flex-wrap gap-6 shadow-inner">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Initial count
            metaCount.textContent = metaDescInput.value.length;
        });
    </script>
    @endpush
</x-app-layout>