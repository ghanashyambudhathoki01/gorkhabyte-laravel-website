                        ...
                        <div class="hidden md:flex md:items-center space-x-5">
                                ...
                                <div class="flex items-center space-x-3">
                                    @if(Auth::user()->isAdmin())
                                        <a href="{{ route('dashboard') }}" 
                                           class="px-4 py-2.5 rounded-xl text-sm font-bold bg-amber-500 text-white hover:bg-amber-600 transition-all shadow-md flex items-center">
                                            Admin Panel
                                        </a>
                                    @endif
                                    
                                    <a href="{{ Auth::user()->isStudent() ? route('student.dashboard') : route('student.register') }}" 
                                       class="px-4 py-2.5 rounded-xl text-sm font-bold bg-indigo-600 text-white hover:bg-indigo-700 transition-all shadow-md flex items-center">
                                        Student Portal
                                    </a>

                                    <form method="POST" action="{{ route('logout') }}" class="inline">
                                        @csrf
                                        <button type="submit" class="text-xs font-semibold text-gray-400 hover:text-red-600 transition-colors uppercase tracking-wider ps-2">
                                            Log Out
                                        </button>
                                    </form>
                                </div>
                            @else
                                <a href="{{ route('student.register') }}" 
                                   class="px-5 py-2.5 rounded-xl text-sm font-bold bg-indigo-600 text-white hover:bg-indigo-700 transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5 flex items-center">
                ...
                <div class="px-2 pt-2 pb-3 space-y-1 bg-white dark:bg-gray-800">
                    ...
                    <div class="pt-4 pb-1 border-t border-gray-100 dark:border-gray-700 space-y-2">
                        @auth
                            @if(Auth::user()->isAdmin())
                                <a href="{{ route('dashboard') }}" 
                                   class="block px-3 py-3 rounded-xl text-base font-bold bg-amber-500 text-white text-center shadow-md">
                                    Admin Panel
                                </a>
                            @endif
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="block w-full text-center px-3 py-2 rounded-lg text-sm font-semibold text-gray-500 dark:text-gray-400 hover:text-red-600">
                                    Log Out
                                </button>
                            </form>
                        @else
